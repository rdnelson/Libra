...
Instruction* Adc::CreateInstruction(Memory::MemoryOffset& memLoc, Processor* proc) {
	...
	switch(*opLoc) {
		case ADC_AL_BYTE:
			...
			newAdc->SetProc(mProc);
			newAdc->SetOperand(Operand::SRC, new ImmediateOperand(*(opLoc + 1), 1, (opLoc + 1).getOffset()));
			newAdc->SetOperand(Operand::DST, new RegisterOperand(Processor8086::REG_AL, mProc));
		...
		case ADC_AX_WORD:
			...
			newAdc->SetOperand(Operand::DST, new RegisterOperand(Processor8086::REG_AX, mProc));
...
int Adc::Execute() {
	...
	unsigned int newVal = dstVal + srcVal + (mProc->GetFlag(Processor8086::FLAGS_CF) ? 1 : 0);
